...
- (id)initWithFrame:(CGRect)frame groupId:(NSString *)groupId index:(NSUInteger)index
{
    ...
    if (self)
    {
        ...
        self.index = index;
        [IRRadioButton registerInstance:self withGroupID:self.groupId];
    }
    return  self;
}
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        ...
        [self.radioButton setImage:[UIImage imageNamedForCurrentBundle:@"RadioButton-Unselected"] forState:UIControlStateNormal];
        [self.radioButton setImage:[UIImage imageNamedForCurrentBundle:@"RadioButton-Selected"] forState:UIControlStateSelected];
...
+ (void)registerInstance:(IRRadioButton *)radioButton withGroupID:(NSString *)aGroupID
{
    if(!rb_instances)
    {
...
- (void)setSelected:(BOOL)selected
{
    if (selected)
    {
        ...
        for (IRRadioButton *radioButton in radioButtons)
        {
...
- (void)radioButtonClicked:(UIButton *)sender
{
    ...
    if (rb_observers)
    {
        ...
        if([observer respondsToSelector:@selector(ir_RadioButtonSelectedAtIndex:inGroup:)])
        {
